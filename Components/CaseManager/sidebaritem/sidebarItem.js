...
class SidebarItemComponent extends React.Component {
  ...
  render() {
    ...
    const updated = _note.timestamp != null ? new Date(_note.timestamp.toDate()).toLocaleDateString() : null
        ...
        <ListItem
          ...
          alignItems='flex-start'>
            <div className={classes.textSection} onClick={() => this.selectNote(_note, _index)}>
                 ...
                 <h4 className={classes.itemText} style={{ width: '10%'}}>{_note.etapa}</h4>
                 <h4 className={classes.itemText} style={{ width: '10%'}}>{updated}</h4>
                 <h4 className={classes.itemText} style={{ width: '25%'}}>{_note.descr}</h4>
            </div> 
