...
if(date.getDate() == ((this.state.fechaInfoViaElect) ? (this.state.fechaInfoViaElect).getDate() : null ) ||!this.state.fechaInfoViaElect ){
     this.setState({fechaInfoViaElect: date,fechaIVEDatabase:date})
     }else{
     this.setState({fechaInfoViaElect: date,fechaIVEDatabase:date, updateIVED: true})
     }
}
handleAudieConci(date){
  if(date.getDate() == ((this.state.fechaAudConcil) ? (this.state.fechaAudConcil).getDate() : null ) ||!this.state.fechaAudConcil ){
     this.setState({fechaAudConcil: date, fechaACDatabase: date})
  }else{
    this.setState({fechaAudConcil: date, fechaACDatabase: date, updateACD: true})
  }
}
...
save = async () =>{
 ...
 if(this.state.fechaIVEDatabase){
   if(this.state.updateIVED){
     await firebase.firestore().collection('fechas').doc(this.state.idIVED).update({
           text: `Caso ${this.props.selectedNote.title} Fecha Informe via Electronico`,
           fecha: this.state.fechaIVEDatabase,
           timestamp: firebase.firestore.FieldValue.serverTimestamp()
        });
      }else{
       const  idIVED = await firebase.firestore()
           .collection('fechas')
           .update({
               text: `Caso ${this.props.selectedNote.title} Fecha Informe via Electronico`,
               fecha: this.state.fechaIVEDatabase,
           ...
           this.setState({idIVED: idIVED.id})
      }
    }
    ...
    if(this.state.fechaACDatabase){
      if(this.state.updateACD){
         await firebase.firestore().collection('fechas').doc(this.state.idACD).update({
                  text: `Caso ${this.props.selectedNote.title} Fecha Audiencia Conciliatora`,
                  fecha: this.state.fechaACDatabase,
                  timestamp: firebase.firestore.FieldValue.serverTimestamp()
        });
     }else{
        const idACD = await firebase.firestore()
              .collection('fechas')
              .add({
                  text: `Caso ${this.props.selectedNote.title} Fecha Audiencia Conciliatora`,
                  fecha: this.state.fechaACDatabase,
...
await firebase.firestore().collection('notes').doc(this.props.selectedNote.id).update({
       'fase2': {
        ...
        idIVED:  this.state.idIVED ? this.state.idIVED  : '',
        coment2: this.state.coment2,
        fechaAudConcil: this.state.fechaAudConcil,
        idACD: this.state.idACD ? this.state.idACD  : '',
